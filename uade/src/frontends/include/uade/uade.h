 ...
 * spawn: 1 if the state was created with spawn=1, 0 otherwise
 * user_data: thread wrapper data passed to thread callbacks
 */
 ...
 * have to.
 */
int uade_read_notification(struct uade_notification* notification, struct uade_state* state);
 ...
 *
 * spawn: 1 to spawn uadecore thread, 0 for analysis-only state
 * user_data: thread wrapper data passed to thread callbacks
 */
...
int uade_song_info(char* info, size_t maxlen, const char* filename, enum uade_song_info_type type);
...
const struct uade_ext_to_format_version* uade_file_ext_to_format_version(const struct uade_detection_info* info);
