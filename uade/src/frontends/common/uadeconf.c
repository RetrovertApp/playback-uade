...
static const struct uade_conf_opts uadeconfopts[] = {
	...
	{.str = "magic_detection",       .l = 2,  .e = UC_CONTENT_DETECTION},
	{.str = "mutemask",              .l = 2,  .e = UC_MUTEMASK},
...
void uade_config_set_defaults(struct uade_config *uc)
{
	...
	uc->timeout = -1;
	uc->use_timeouts = 1;
	uc->write_audio_fd = -1;  /* invalid fd value */
}
...
{
#define MERGE_OPTION(y) do { if (ucs->y##_set) ucd->y = ucs->y; ucd->y##_set = 1; } while (0)
...
#define SET_OPTION(opt, value) do { uc->opt = (value); uc->opt##_set = 1; } while (0)
	...
	case UC_MUTEMASK:
		...
		for (size_t i = 0;; i++) {
			char digit = value[i];
			if ('0' <= digit && digit <= '3') {
				uc->mutemask |= 1 << (digit - '0');
			} else if (digit == '\0') {
				break;
			} else {
				uade_die("uade.conf: expected string consisting of characters 0, 1, 2 or 3. "
					 "Got %s\n", value);
			}
		}
		uc->mutemask_set = 1;
	...
	case UC_WRITE_AUDIO_FD:
		x = strtol(value, &endptr, 10);
		if (*endptr != 0 || x < 0) {
			fprintf(stderr, "Invalid fd number: %s\n", value);
			break;
		}
		SET_OPTION(write_audio_fd, x);
		break;
