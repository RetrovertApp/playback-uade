...
#if !defined(__CYGWIN__) && !defined(__MINGW32__) && !defined(_WIN32) && !defined(__AMIGA__) && !defined(__AROS__) && !defined(__wasm__) && !defined(__ANDROID__) && !defined(__serenity__) && !defined(__vita__)
...
{
	for (int i = 0; i < state->songdb.nsongs; ++i) {
		struct eaglesong *s = &state->songdb.songstore[i];
		struct uade_attribute *a = s->attributes;
		while (a != NULL) {
			struct uade_attribute *next = a->next;
			free(a->s);
			free(a);
			a = next;
		}
