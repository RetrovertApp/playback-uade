...
#include <sys/types.h>
#include <time.h>
#ifndef DISABLE_BENCODETOOLS
#include <bencodetools/bencode.h>
#endif

#define ASSERT_RECEIVE_STATE(state)                 \
    do {                                            \
        assert((state)->ipc.state == UADE_R_STATE); \
    } while (0)
#define ASSERT_SEND_STATE(state)                    \
    do {                                            \
...
static void set_ipc_state(struct uade_state* state, enum uade_control_state newstate) {
...
static void set_end_event(struct uade_event* event, int tailbytes, int happy, int stopnow, const char* reason,
                          struct uade_state* state) {
    struct uade_subsong_info* subsongs = &state->song.info.subsongs;
    int lastsubsong = (subsongs->cur >= subsongs->max) || (subsongs->cur < subsongs->min);
...
static void handle_send_uadecore_logs(struct uade_state* state, const struct uade_msg* um) {
...
void uade_set_amiga_loader(struct uade_file* (*amigaloader)(const char* name, const char* playerdir, void* context,
                                                            struct uade_state* state),
...
static int send_file_back(struct uade_file* f, const char* name, struct uade_state* state) {
...
static void set_subsong(struct uade_state* state) {
    ...
    if (state->song.nextsubsongtrigger) {
        ...
        if (state->song.seekmode == UADE_SEEK_SUBSONG_RELATIVE || state->song.seekmode == UADE_SEEK_POSITION_RELATIVE)
...
static void set_subsong_and_seek(int whence, int subsong, int64_t seekoffs, struct uade_state* state) {
    state->song.seekmodetrigger = whence;
    state->song.seeksubsongtrigger = subsong;
    state->song.seekoffstrigger = seekoffs;
}

static int64_t samples_to_offset(ssize_t samples, const struct uade_state* state) {
    return ((int64_t)samples) * UADE_BYTES_PER_FRAME;
}

static int seek_subsong_relative(ssize_t samples, int subsong, struct uade_state* state) {
...
int uade_seek(enum uade_seek_mode whence, double seconds, int subsong, struct uade_state* state) {
...
int uade_seek_samples(enum uade_seek_mode whence, ssize_t samples, int subsong, struct uade_state* state) {
    switch (whence) {
        case UADE_SEEK_SONG_RELATIVE:
...
static int queue_command(struct uade_state* state, void* command, size_t size) {
    if (ipc_state(state) == UADE_S_STATE)
        return uade_send_message((struct uade_msg*)command, &state->ipc);
    if (state->write_queue == NULL)
...
double uade_get_time_position(enum uade_seek_mode whence, const struct uade_state* state) {
    int64_t bytes;
...
static int test_timeouts(struct uade_event* event, struct uade_state* state) {
    ...
    if (state->config.timeout >= 0 && (state->song.info.songbytes / bytespersec) >= state->config.timeout) {
        dont_record_playtime(state);
    ...
    while (1) {
        ...
        switch (event->type) {
            ...
            case UADE_EVENT_READY:
                ...
                if (state->song.nextsubsongtrigger || state->song.seekmodetrigger) {
                    set_subsong(state);
                }
...
// disable warnings from scan-build since as per UADE API it's developers responsibility to free the memory to avoid
// leak from strdup
...
static void notify_song_end(struct uade_state* state, const struct uade_event_songend* song_end) {
...
int uade_read_notification(struct uade_notification* n, struct uade_state* state) {
...
struct uade_state* uade_new_state(const struct uade_config* extraconfig, int spawn, void* user_data) {
    ...
    /* Only spawn uadecore if requested */
    if (spawn) {
...
const struct uade_config* uade_get_const_effective_config(const struct uade_state* state) {
    return &state->config;
}
...
static struct eagleplayer* get_eagleplayer(struct uade_file* module, struct uade_state* state) {
    struct uade_detection_info* detectioninfo = &state->song.info.detectioninfo;
...
static int uade_play_internal(struct uade_file* module, int subsong, struct uade_state* state) {
    ...
    if (subsong >= 0)
        set_subsong_and_seek(UADE_SEEK_SUBSONG_RELATIVE, subsong, 0, state);
    ...
    if (module->name != NULL)
        strlcpy(song->info.modulefname, module->name, sizeof song->info.modulefname);
    ...
    if (player->name != NULL)
        strlcpy(song->info.playerfname, player->name, sizeof song->info.playerfname);
    ...
    while (1) {
        ...
        switch (event.type) {
            ...
            case UADE_EVENT_PLAYER_NAME:
                strlcpy(song->info.playername, event.msg, sizeof song->info.playername);
                break;
            case UADE_EVENT_FORMAT_NAME:
                strlcpy(song->info.formatname, event.msg, sizeof song->info.formatname);
                break;
            case UADE_EVENT_MODULE_NAME:
                strlcpy(song->info.modulename, event.msg, sizeof song->info.modulename);
                break;
...
int uade_set_song_options(const char* songfile, const char* songoptions, struct uade_state* state) {
    ...
    snprintf(homesongconfname, sizeof homesongconfname, "%s/.uade/song.conf", home);
...
int uade_stop(struct uade_state* state) {
    ...
    if (state->song.recordsongtime && state->song.state == UADE_STATE_WAIT_SUBSONG_CHANGE) {
