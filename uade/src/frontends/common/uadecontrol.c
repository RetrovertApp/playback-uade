...
static void send_mute_command(struct uade_ipc *ipc,
				    struct uade_config *uadeconf)
{
	if (uade_send_u32(UADE_COMMAND_SET_MUTE_MASK, uadeconf->mutemask, ipc))
		uade_warning("Can not set mute mask\n");
}
					  ...
					  struct uade_config *uadeconf)
{
	...
	if (strlen(fname) > 0) {
		assert(uadeconf->write_audio_fd < 0);
		if (uade_send_string(UADE_COMMAND_SET_WRITE_AUDIO_FNAME, fname,
				     ipc))
			uade_warning("Can not set write audio fname\n");
	} else if (uadeconf->write_audio_fd >= 0) {
		if (uade_send_u32(UADE_COMMAND_SET_WRITE_AUDIO_FD,
				  (uint32_t) uadeconf->write_audio_fd, ipc))
			uade_warning("Can not set write audio fd\n");
	}
}
