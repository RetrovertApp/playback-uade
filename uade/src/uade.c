...
/* REMOVED: static const int SCORE_MODULE_NAME_ADDR = 0x128; */
...
static const int SCORE_PATH_ARRAY_ADDR = 0x00400;  /* 512 bytes reserved */
static const int SCORE_DIR_ARRAY_ADDR = 0x00600;  /* 256 bytes reserved */
static const int SCORE_FILE_ARRAY_ADDR = 0x00700;  /* 256 bytes reserved */
...
static const char *AMIGADIR = "UADE:";
static char cacheddirname[PATH_MAX - 1];
...
static struct uade_file *lookup_amiga_file_cache(const char *filename)
{
	...
	// replace UADE:* with /<realpathname>/*
	// this would perhaps belong to uade_find_amiga_file,
	// but would break the current amiga loader wrapper API
	if (!strncmp(AMIGADIR, filename, strlen(AMIGADIR))) {
		strlcat(realfilename, cacheddirname, sizeof realfilename);
		if (strnlen(filename, strlen(AMIGADIR) + 1) > strlen(AMIGADIR)) {
			strlcat(realfilename, "/", sizeof realfilename);
			strlcat(realfilename, filename + strlen(AMIGADIR), sizeof realfilename);
		}
	} else {
		strlcpy(realfilename, filename, sizeof realfilename);
	}

	/* Return cached file, if filename matches */
	if (cachedfile != NULL && strcmp(realfilename, cachedfilename) == 0)
		return cachedfile;

	f = uade_request_amiga_file(realfilename, &uadecore_ipc);
	if (f == NULL)
		return NULL;
	...
	cachedfile = f;
	strlcpy(cachedfilename, realfilename, sizeof cachedfilename);
...
// convert socket handles to proper fds
  ...
  if (_in_fd < 0) {
    fprintf(stderr, "uadecore: Could not open read socket handle %d (%s)\n", in_fd, strerror(errno));
    exit(1);
  }
  int _out_fd = _in_fd;
  if (in_fd != out_fd) {
    _out_fd = _open_osfhandle(out_fd, O_RDWR|O_BINARY);
    if (_out_fd < 0) {
      fprintf(stderr, "uadecore: Could not open write socket handle %d (%s)\n", out_fd, strerror(errno));
      exit(1);
    }
  }
  uade_set_peer(&uadecore_ipc, 0, _in_fd, _out_fd);
#elif defined(__AMIGA__) || defined(__AROS__)
  // -i and -o give the unique pipe id
  char in_pipe[32];
  char out_pipe[32];
  snprintf(in_pipe, sizeof in_pipe, "PIPE:uade-%d", in_fd);
  snprintf(out_pipe, sizeof out_pipe, "PIPE:uadecore-%d", out_fd);
  BPTR _out_fd = Open(out_pipe, MODE_NEWFILE);
  if (!_out_fd) {
    fprintf(stderr, "uadecore: Could not open write pipe %s (%d)\n", out_pipe, IoErr());
    exit(1);
  }
  // create pipe file
  if (Write(_out_fd, out_pipe, 1) != 1) {
    fprintf(stderr, "uadecore: Could not write to pipe %s (%d)\n", out_pipe, IoErr());
	Close(_out_fd);
	exit(1);
  }
  BPTR _in_fd = Open(in_pipe, MODE_OLDFILE);
  if (!_in_fd) {
    fprintf(stderr, "uadecore: Could not open read pipe %s (%d)\n", in_pipe, IoErr());
	Close(_out_fd);
    exit(1);
  }
  // drain dummy data
  if (Read(_in_fd, in_pipe, 1) != 1) {
    fprintf(stderr, "uadecore: Could not read from pipe %s (%d)\n", in_pipe, IoErr());
	Close(_out_fd);
	Close(_in_fd);
	exit(1);
  }
...
void uadecore_reset(void)
{
 ...
 nextsong:
  ...
  if (module->data != NULL) {
	  ...
	  modulepath = song.modulename;
  } else {
	  modulepath = song.playername;
  }

  char *filename = strrchr(modulepath, (int) '/');
  if (!filename)
    filename = modulepath;
  else
    filename++; 

  uade_dirname(cacheddirname, modulepath, sizeof cacheddirname);

  char patharray[512];
  patharray[0] = 0;

  strlcpy(patharray, AMIGADIR, sizeof patharray);
  assert(strlen(filename) < 512);
  strlcat(patharray, filename, sizeof patharray);

  if (!valid_address(SCORE_PATH_ARRAY_ADDR, strlen(patharray) + 1) ||
	  !valid_address(SCORE_DIR_ARRAY_ADDR, strlen(AMIGADIR) + 1) ||
	  !valid_address(SCORE_FILE_ARRAY_ADDR, strlen(filename) + 1)) {
	  fprintf(stderr, "uadecore: Invalid address for module/playername.\n");
	  goto skiptonextsong;
  }

  strlcpy((char *) get_real_address(SCORE_PATH_ARRAY_ADDR), patharray, 512);
  strlcpy((char *) get_real_address(SCORE_DIR_ARRAY_ADDR), AMIGADIR, 256);
  strlcpy((char *) get_real_address(SCORE_FILE_ARRAY_ADDR), filename, 256);
